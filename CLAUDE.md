...
- На каждый тикет — один файл `docs/tasks/SF-{N}.md`: чек-лист acceptance criteria, как реализовано (по темам), как протестировать, краткая документация по затронутым темам — для быстрого восстановления контекста между сессиями.
